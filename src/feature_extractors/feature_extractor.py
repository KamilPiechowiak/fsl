...
class FeatureExtractor:
    ...
    def extract_features(self, dataset: torch.utils.data.DataLoader) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            for x, y in tqdm(dataset):
                x, y = x.to(self.DEVICE), y.to(self.DEVICE)
                features += [feature_extractor(x).cpu().numpy().reshape(len(y),-1)]
    ...
    def load(self, filename: str, path=None) -> None:
        ...
        if self.config['model'].get('load_optimizer', 0) == 1:
            if hasattr(self, "optimizer"):
                self.optimizer.load_state_dict(checkpoint['optimizer'])
            if self.config['model'].get('load_scheduler', 1) == 1 and hasattr(self, "scheduler"):
